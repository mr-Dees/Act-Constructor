 ...
 * Отвечает за рендеринг финальной версии акта для пользователя
 */
class PreviewManager {
    /**
     * Обновляет содержимое панели предпросмотра
     * @param {Object} options - Настройки отображения
     * @param {number} options.previewTrim - Максимальная длина текста для обрезки
     */
     ...
     * @param {number} level - Уровень вложенности (для заголовков)
        ...
        node.children.forEach(child => {
            ...
            if (node.type === 'textblock') {
                ...
                if (textBlock) {
                    ...
                    // Применяем сохраненное форматирование текста
                    ...
                    // Вставляем HTML-контент с переносами строк
            ...
            // Рендеринг нарушения в текстовом формате
            if (child.type === 'violation') {
                ...
                if (violation) {
                    ...
                    // Список описаний с подсчетом метрик
                    if (violation.descriptionList.enabled && violation.descriptionList.items.length > 0) {
                        ...
                        if (itemsCount > 0) {
                            ...
                            // Правильное склонение числительного
