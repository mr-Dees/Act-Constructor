...
| `chat` | Активный | AI-ассистент (conversations, SSE-стриминг, function-calling, файлы) |
...
**Shared (`static/js/shared/` — 11 модулей):**
...
| `chat/chat-manager.js` | Главный менеджер чата (оркестрирует stream, renderer, history) |
...
- Portal JS: `portal-sidebar.js`, `chat-stream.js`, `chat-renderer.js`, `chat-history.js`, `chat-manager.js`, `chat-modal.js`, `portal-settings.js`
...
На текущий момент knowledge bases собираются на фронтенде (`ChatManager._getEnabledKnowledgeBases()`), но не передаются в API. Это место для будущей RAG-интеграции.
...
Клиент собирает выбранные базы знаний на фронтенде (`ChatManager._getEnabledKnowledgeBases()`), но текущий API не принимает их в запросе. Это место для будущей RAG-интеграции.
