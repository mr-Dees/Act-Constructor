...
Форматтеры используют общие утилиты из `app/formatters/utils/`:

| Утилита | Файл | Назначение |
|---------|------|-----------|
| `base_formatter.py` | Базовый класс форматтеров | Общий интерфейс и обход дерева |
...
Vanilla JS (ES6+) без фреймворков и бандлеров. ~90 модулей, загружаемых через `<script>` теги в шаблонах.
