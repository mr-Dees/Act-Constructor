 ...
 * поддерживают структурные операции (вставка/удаление колонки) и пересчёт в
 * проценты для colgroup в редакторе — всё без DOM, чтобы тестировать чистую
 * логику и гарантировать, что pydantic (colWidths: list[int]) не получит дробей
 * и не отдаст 422.
 */
...
test('property: insert/remove сохраняют целые веса >=1', () => {
