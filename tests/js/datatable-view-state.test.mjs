...
test('resetToDefault сбрасывает и extra-флаги (и это переживает перезагрузку)', () => {
  const st = fakeStorage();
  const s = make(st);
  s.setExtra('tbView', 'pivot');
  assert.equal(s.getExtra('tbView', 'chips'), 'pivot');
  s.resetToDefault();
  assert.equal(s.getExtra('tbView', 'chips'), 'chips'); // extra очищен → дефолт
  const s2 = make(st); // «перезагрузка страницы»: сброс не откатился к сохранённому extra
  assert.equal(s2.getExtra('tbView', 'chips'), 'chips');
});
