 ...
 *     (colSpan/rowSpan===1, isSpanned===false) и восстанавливает содержимое
 *     всех ячеек области из mergeSnapshot;
...
test('property: round-trip merge→unmerge разъединяет прямоугольник и восстанавливает содержимое', () => {
  fc.assert(
    fc.property(arbGridAndRect, ({ grid, r1, c1, r2, c2 }) => {
      const merged = mergeRange(grid, r1, c1, r2, c2);
      const restored = unmergeAt(merged, r1, c1);

      // Все ячейки прямоугольника снова синглтоны без span-меток, а их
      // содержимое восстановлено из mergeSnapshot (склейку не редактировали).
      for (let r = r1; r <= r2; r++) {
        for (let c = c1; c <= c2; c++) {
          ...
          assert.equal(cell.content, grid[r][c].content, `content не восстановлен в (${r},${c})`);
        }
      }

      // Снапшот снимается при unmerge — не должен «протухать» в модели.
      assert.ok(!('mergeSnapshot' in restored[r1][c1]), 'mergeSnapshot остался на ведущей');
    }),
    { numRuns: 300 },
  );
});
