...
test('«ТБ, выявившие отклонение» (tb_breakdown): словарный фильтр-резолвер, noSort, render — чипы; пустая развертка → «—»', () => {
  ...
  assert.equal(col.type, 'dictionary'); // без типа словаря хедер-UI не применит filterResolve
