...
test('npl_breakdown: автовыведенная колонка скрыта по умолчанию, без сортировки/фильтра на бэке (спека не делает чипы/пивот по NPL — см. design §1.3)', () => {
...
test('npl_breakdown: заглушка на случай принудительной видимости («Выбрать все» обходит default-hidden) — не «[object Object]»; label отличается от лейбла поля формы', () => {
  const col = CkFinResConfig.columns.find((c) => c.key === 'npl_breakdown');
  ...
  assert.notEqual(col.label, 'NPL 90+, руб.', 'лейбл служебной колонки не должен совпадать с лейблом поля формы / будущей колонки total_npl_amount');
});
...
test('total_npl_amount: render 0/null → «—», 120000.5 → форматированные деньги', () => {
