 ...
 * НЕ-кейсе (image/freeText). Это единственная реализация правила — форма
 * (violation-rendering.js) и drag-миниатюра (violation-drag-drop.js)
 * используют её напрямую, без DOM-дубля.
 */
...
test('два кейса, затем image, затем кейс: 1, 2, (image), 1 — сценарий формы и drag-миниатюры', () => {
    const items = [
        { id: 'c1', type: 'case', content: 'первый' },
        { id: 'c2', type: 'case', content: 'второй' },
        { id: 'img1', type: 'image', url: 'data:image/png;base64,AAAA' },
        { id: 'c3', type: 'case', content: 'третий' },
    ];
    const result = computeAdditionalContentNumbers(items);
    assert.deepEqual(result.map((r) => r.number), [1, 2, null, 1]);
});
