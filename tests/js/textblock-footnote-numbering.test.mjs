...
// ── Task 7 (TREE-1): renumberAllFootnotes — глобальный проход по листу ───────

/**
 * Фейковый редактор: querySelectorAll('.text-footnote') → свой список сносок;
 * __lastFootnoteCount отслеживаем для проверки когерентности кэша finalizeEdit.
 */
function fakeEditor(footnotes) {
  return {
    __lastFootnoteCount: 99, // заведомо неверное значение — проход обязан примирить
    querySelectorAll: (sel) => (sel === '.text-footnote' ? footnotes : []),
  };
}

/** Фейковый #itemsContainer: диспетчит querySelectorAll по селектору. */
function fakeContainer(footnotesInDocOrder, editors) {
  return {
    querySelectorAll: (sel) => {
      if (sel === '.text-footnote') return footnotesInDocOrder;
      if (sel === '.textblock-editor') return editors;
      return [];
    },
  };
}

test('renumberAllFootnotes: сквозная нумерация 1,2,3 по нескольким редакторам + кэш __lastFootnoteCount примирён', () => {
  const mgr = Object.create(TextBlockManager.prototype);
  const fA = [fakeFootnote('a')];
  const fB = [fakeFootnote('b1'), fakeFootnote('b2')];
  const edA = fakeEditor(fA);
  const edB = fakeEditor(fB);
  // document order контейнера: сначала все сноски A, затем B (как в дереве/превью).
  const container = fakeContainer([...fA, ...fB], [edA, edB]);

  mgr.renumberAllFootnotes(container);

  assert.deepEqual(fA.map((x) => x.num()), ['1'], 'блок A: сноска №1');
  assert.deepEqual(fB.map((x) => x.num()), ['2', '3'], 'блок B: сквозные №2,№3');
  assert.equal(edA.__lastFootnoteCount, 1, 'кэш A = число .text-footnote');
  assert.equal(edB.__lastFootnoteCount, 2, 'кэш B = число .text-footnote');
});

test('renumberAllFootnotes: пустая сноска пропускается в сквозном счёте, но входит в кэш редактора', () => {
  const mgr = Object.create(TextBlockManager.prototype);
  const empty = fakeFootnote('   ');
  const f = [fakeFootnote('a'), empty, fakeFootnote('b')];
  const ed = fakeEditor(f);
  mgr.renumberAllFootnotes(fakeContainer(f, [ed]));
  assert.equal(f[0].num(), '1');
  assert.equal(empty.num(), undefined, 'пустая — без номера (паритет с превью/бэком)');
  assert.equal(f[2].num(), '2', 'нумерация сквозная по непустым');
  // Кэш гейта finalizeEdit считает ВСЕ .text-footnote (включая пустые) — 3.
  assert.equal(ed.__lastFootnoteCount, 3, 'кэш = все капсулы (тот же критерий, что finalizeEdit)');
});

test('renumberAllFootnotes: нет контейнера / без querySelectorAll → no-op без исключения', () => {
  const mgr = Object.create(TextBlockManager.prototype);
  assert.doesNotThrow(() => mgr.renumberAllFootnotes(null));
  assert.doesNotThrow(() => mgr.renumberAllFootnotes({}));
});
