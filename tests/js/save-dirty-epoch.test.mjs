...
// ─── A1/#14: единый финал finalizeDbSave ────────────────────────────────────

test('A1: finalizeDbSave при выросшей эпохе НЕ синхронизирует, но гасит offline-машинерию', () => {
  let removedOnline = 0;
  const origRemove = globalThis.removeEventListener;
  globalThis.removeEventListener = () => { removedOnline++; };
  try {
    // Предыстория: был сбой сохранения — взведён дедуп-флаг + подписка 'online'.
    StorageManager._dbSaveFailureNotified = true;
    StorageManager._onlineRetryHandler = () => {};
    StorageManager._dirtyEpoch = 5;
    StorageManager._setState('unsaved');
    removeCalls = 0;

    StorageManager.finalizeDbSave(7, 3); // эпоха 5 ≠ снятая 3 → правки во время PUT

    assert.notEqual(StorageManager._state, 'saved', 'эпоха выросла → НЕ синхронизирован');
    assert.equal(removeCalls, 0, 'снимок-черновик не снят');
    assert.equal(StorageManager._dbSaveFailureNotified, false,
      'A1: PUT прошёл — дедуп-флаг сброшен, следующий реальный сбой снова предупредит');
    assert.equal(StorageManager._onlineRetryHandler, null, 'A1: подписка online снята (сервер достижим)');
    assert.equal(removedOnline, 1, 'removeEventListener для online вызван');
  } finally {
    if (origRemove) globalThis.removeEventListener = origRemove;
    else delete globalThis.removeEventListener;
  }
});

test('A1/#14: finalizeDbSave при стабильной эпохе синхронизирует и снимает снимок', () => {
  StorageManager._dbSaveFailureNotified = false;
  StorageManager._onlineRetryHandler = null;
  StorageManager._dirtyEpoch = 4;
  StorageManager._setState('unsaved');
  removeCalls = 0;

  StorageManager.finalizeDbSave(7, 4); // эпоха стабильна

  assert.equal(StorageManager._state, 'saved', 'стабильная эпоха → синхронизирован');
  assert.equal(removeCalls, 1, 'снимок-черновик снят');
});
