...
test('_specFromSingleDate: точная дата → eq, пусто → null', () => {
  const dt = makeTable();
  assert.deepEqual(dt._specFromSingleDate('2025-02-01'), { op: 'eq', value: '2025-02-01' });
  assert.equal(dt._specFromSingleDate(''), null);
});

test('дата-контрол: single — isFilled по строке даты; range — по from/to', () => {
  const dt = makeTable();
  const singleBox = dt._buildFilterControl({ key: 'dt', label: 'Дата СЗ', type: 'date', dateFilter: 'single' });
  assert.equal(singleBox.isFilled(), false);
  dt._filterText.dt = '2025-02-01';
  assert.equal(singleBox.isFilled(), true);

  const rangeBox = dt._buildFilterControl({ key: 'dt2', label: 'Период', type: 'date' });
  assert.equal(rangeBox.isFilled(), false);
  dt._filterText.dt2 = { from: '2025-01-01', to: '' };
  assert.equal(rangeBox.isFilled(), true);
});
