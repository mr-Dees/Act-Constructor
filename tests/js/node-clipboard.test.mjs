 ...
 *  - estimatePastedImageBytes / checkImageLimits: лимит картинок (КП-5).
 *
...
test('estimatePastedImageBytes суммирует только image-элементы', () => {
    const violations = {
    ...
    assert.equal(estimatePastedImageBytes(violations), 300);
    assert.equal(estimatePastedImageBytes({}), 0);
});
...
test('round-trip: копирование и вставка поддерева даёт новые id, ссылки целы', () => {
    ...
    // Вставляем в другой узел (раздел 6 же, но другой пункт).
    const dest = addItem('6', 'Назначение');
    AppState.generateNumbering();
...
test('КП-3: pinned-таблицу (риск) нельзя копировать как корень', () => {
    AppState.initializeTree(true);
    ...
    assert.equal(NodeClipboard.copyNode(riskNode.id), false);
    assert.equal(notified.error.length, 1);
});
...
test('КП-3: pinned-дети поддерева пропускаются при вставке, юзер уведомлён', () => {
    ...
    // Вставляем в раздел 6 (вне §5) — риск-таблица должна быть отброшена.
    const dest = addItem('6', 'Назначение');
    AppState.generateNumbering();
...
test('КП-5: вставка отклонена при превышении лимита картинок акта', () => {
    AppState.initializeTree(true);
    const src = addItem('6', 'С картинкой');
...
test('КП-6: вставка внутрь листового блока (таблицы) запрещена', () => {
    ...
    const holder = addItem('6', 'Держатель');
...
test('КП-6: вставка с превышением maxDepth отклонена', () => {
    ...
    // 6 (depth1) → d2 → d3 ; вставка a сюда даст 4+ уровней.
    const d2 = addItem('6', 'd2');
