...
class DocxFormatter(BaseFormatter):
    ...
    def __init__(self, settings: Settings):
        ...
        self.MAX_IMAGE_SIZE_MB = fmt.max_image_size_mb
        self.HTML_PARSE_TIMEOUT = fmt.html_parse_timeout
        self.MAX_HTML_DEPTH = fmt.max_html_depth
        self.HTML_PARSE_CHUNK_SIZE = fmt.html_parse_chunk_size
        # Параметры retry логики
        self.MAX_RETRIES = fmt.max_retries
        self.RETRY_DELAY = fmt.retry_delay
        ...
        logger.debug(f"DocxFormatter инициализирован с настройками: "
                     f"image_width={fmt.docx_image_width}\", "
                     f"max_image_size={fmt.max_image_size_mb}MB, "
                     f"parse_timeout={fmt.html_parse_timeout}s, "
                     f"chunk_size={fmt.html_parse_chunk_size}, "
                     f"max_retries={fmt.max_retries}")
