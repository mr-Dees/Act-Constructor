...
(established/violated/reasons/responsible/consequences). Меры вычисляются, но в
карточку НЕ пишутся (поля measures в схеме нет — по решению владельца). Структуру
JSON получаем провайдер-агностично (промпт → JSON → разбор), БЕЗ ``response_format``.
