"""Фича «Корректор»: правка орфографии/пунктуации выделенного текста (one-shot).

Перенос наработки D17 (папка 1) на нативную LLM-инфру домена чата:
``build_llm_client`` + ``retry_on_transient`` вместо vLLM/LangChain. Промпт —
дословный (``AUDITOR_SYSTEM_PROMPT``), логика вызова упрощена до одного
...
from app.domains.chat.services.text_actions.prompts import AUDITOR_SYSTEM_PROMPT
from app.domains.chat.settings import ChatDomainSettings


class TextCorrectorService:
    """Прогоняет выделенный текст через корректорский промпт D17."""
    ...
    def __init__(self, settings: ChatDomainSettings) -> None:
        ...
        self._temperature = ta.corrector_temperature
    ...
    async def correct(self, text: str) -> str:
        """Вернуть исправленный текст. Кидает ``TextActionValidationError``
        на пустой/слишком длинный ввод."""
