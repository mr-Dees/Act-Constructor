...
class KMUtils:
    ...
    def extract_km_digits(km_number: str) -> str:
        ...
        Ожидается ровно 6 цифр.

        Args:
            km_number: КМ в формате КМ-XX-XXXX или произвольная строка.

        Returns:
            Строка из 6 цифр.

        Raises:
            ValueError: если количество цифр не равно 6.
        """
        digits = re.sub(r"[^0-9]", "", km_number)

        if len(digits) != 6:
            raise ValueError(
                f"КМ номер должен содержать ровно 6 цифр, получено: "
