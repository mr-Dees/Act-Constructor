...
async def example_structure():
    ...
    print("Выполняется запрос: get_act_structure('КМ-11-11111', with_statistics=False)")

    structure = await get_act_structure("КМ-11-11111", with_statistics=False)
    print("\n📊 Структура акта:")
    print_result_preview(structure, 600)

    # Структура со статистикой
    print_subsection_header("Структура со статистикой по элементам")
    print("Выполняется запрос: get_act_structure('КМ-11-11111', with_statistics=True)")

    structure_stats = await get_act_structure("КМ-11-11111", with_statistics=True)
    ...
    print("Выполняется запрос: get_act_structures_batch(['КМ-11-11111', 'КМ-22-22222'])")

    structures = await get_act_structures_batch(
        ["КМ-11-11111", "КМ-22-22222"],
...
async def example_full_acts():
    ...
    print("Выполняется запрос: get_act_by_km('КМ-11-11111', with_metadata=True)")

    act = await get_act_by_km("КМ-11-11111", with_metadata=True)
    print("\n📊 Полный текст акта:")
    print_result_preview(act, 800)

    # Один акт без метаданных
    print_subsection_header("Получение акта без метаданных")
    print("Выполняется запрос: get_act_by_km('КМ-11-11111', with_metadata=False)")

    act_no_meta = await get_act_by_km("КМ-11-11111", with_metadata=False)
    ...
    print("Выполняется запрос: get_acts_by_km_list(['КМ-11-11111', 'КМ-22-22222'])")
...
async def example_items():
    ...
    print("Выполняется запрос: get_item_by_number('КМ-11-11111', '5.1.1', recursive=True)")
    ...
    print("Выполняется запрос: get_item_by_number('КМ-11-11111', '5.1.1', recursive=False)")
    ...
    print("Выполняется запрос: get_item_by_number('КМ-11-11111', '5.1', max_depth=2)")
    ...
    print("Выполняется запрос: get_item_by_number('КМ-11-11111', ['5.1.1', '5.1.2'])")
...
async def example_violations():
    ...
    print("Выполняется запрос: get_all_violations('КМ-11-11111', with_metadata=True)")

    violations = await get_all_violations("КМ-11-11111", with_metadata=True)
    print("\n📊 Все нарушения:")
    ...
    print("Выполняется запрос: get_violation_by_item('КМ-11-11111', '5.1.1.1', recursive=False)")
    ...
    print("Выполняется запрос: get_violation_by_item('КМ-11-11111', '5.1', recursive=True)")
    ...
    print("Выполняется запрос: get_violation_fields('КМ-11-11111', '5.1', ['case', 'reasons'])")
    ...
    print("Выполняется запрос: get_violation_fields('КМ-11-11111', '5.1.1', ['additional_content'])")
    ...
    print("Выполняется запрос: get_violation_by_item('КМ-11-11111', ['5.1.2', '5.1.1.1'])")
...
async def example_tables():
    ...
    print("Выполняется запрос: get_all_tables('КМ-11-11111', with_metadata=True)")

    tables = await get_all_tables("КМ-11-11111", with_metadata=True)
    print("\n📊 Все таблицы:")
    print_result_preview(tables, 600)

    # Таблицы по пункту
    print_subsection_header("Таблицы в конкретном пункте")
    print("Выполняется запрос: get_all_tables_in_item('КМ-11-11111', '5.1.1.1', recursive=False)")
    ...
    print("Выполняется запрос: get_all_tables_in_item('КМ-11-11111', ['5.1.2', '5.1.1.1'])")
    ...
    print("Выполняется запрос: get_table_by_name('КМ-11-11111', '5.1.1.1', 'операционного риска')")
    ...
    print("Выполняется запрос: get_table_by_name('КМ-11-11111', ['5.1.1.1', '5.1.1.2'], ['операционного', 'регуляторного'])")
...
async def example_textblocks():
    ...
    print("Выполняется запрос: get_all_textblocks('КМ-11-11111', with_metadata=True)")

    textblocks = await get_all_textblocks("КМ-11-11111", with_metadata=True)
    print("\n📊 Все текстовые блоки:")
    print_result_preview(textblocks, 600)

    # Текстовые блоки по пункту
    print_subsection_header("Текстовые блоки в конкретном пункте")
    print("Выполняется запрос: get_textblocks_by_item('КМ-11-11111', '5.1.1.1', recursive=False)")
    ...
    print("Выполняется запрос: get_textblocks_by_item('КМ-11-11111', ['5.1.2', '5.1.1.1'])")
...
async def example_invoices():
    """
    Пример 8: Работа с фактурами.

    Демонстрирует:
    - Все фактуры акта
    - Фактуры по конкретному пункту
    - Batch-обработку по нескольким пунктам
    """
    await ensure_db_initialized()

    print_section_header("ИЗВЛЕЧЕНИЕ ФАКТУР", "📎")

    # Все фактуры акта
    print_subsection_header("Все фактуры акта")
    print("Выполняется запрос: get_all_invoices('КМ-11-11111', with_metadata=True)")

    invoices = await get_all_invoices("КМ-11-11111", with_metadata=True)
    print("\n📊 Все фактуры:")
    print_result_preview(invoices, 600)

    # Фактуры по пункту
    print_subsection_header("Фактуры в конкретном пункте")
    print("Выполняется запрос: get_invoices_by_item('КМ-11-11111', '5.1.1.1', recursive=False)")

    invoices_item = await get_invoices_by_item(
        "КМ-11-11111",
        "5.1.1.1",
        recursive=False
    )
    print("\n📊 Фактуры в пункте 5.1.1.1:")
    print_result_preview(invoices_item, 500)

    # Batch-обработка
    print_subsection_header("Фактуры по нескольким пунктам батчем")
    print("Выполняется запрос: get_invoices_by_item('КМ-11-11111', ['5.1.2', '5.1.1.1'])")

    invoices_batch = await get_invoices_by_item(
        "КМ-11-11111",
        ["5.1.2", "5.1.1.1"],
        recursive=False
    )
    print(f"\n📊 Получено результатов: {len(invoices_batch)}")
    for num, content in invoices_batch.items():
        print(f"\n  🔹 Пункт {num}:")
        print_result_preview(content, 300)
...
if __name__ == "__main__":
    ...
    #         # await example_invoices()    # Фактуры
    #     finally:
    #         await close_db()
