...
Снятие безопасно: колонка больше нигде не читается (экспортёры DOCX/text/markdown
и рендер берут форматирование из `content`), исторические значения `formatting`
не нужны (обратная совместимость не требуется).
