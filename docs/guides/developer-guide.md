...
Аналогично колонка `mpl_amount_rubles NUMERIC(38, 2) DEFAULT 0` («MPL 90+, руб.» — показатель заполняется только для метрики `602`, `MPL_METRIC_CODES` в `fr_validation_service.py`, вручную синхронизирована с одноимённой константой `CkFinResConfig.MPL_METRIC_CODES` в `ck-fin-res-config.js`) добавлена тем же способом после первых развёртываний: на уже развёрнутых PG — ручной `ALTER TABLE t_db_oarb_ck_fr_validation ADD COLUMN mpl_amount_rubles NUMERIC(38, 2) DEFAULT 0` и обязательный повторный `CREATE OR REPLACE VIEW v_db_oarb_ck_fr_validation ...` (VIEW фиксирует star-expansion при создании — без пересоздания новую колонку не увидит); на GP колонку добавляет ETL. `FRValidationService.group_save` проверяет обе стороны правила метрики 602 (MPL заполнен ⇒ метрика 602, метрика 602 ⇒ MPL заполнен), а строка развёртки по ТБ существует, если сумма ИЛИ MPL больше нуля (`TBBreakdownItem._at_least_one_amount` в `schemas/group.py`). Групповой агрегат `total_mpl_amount` (`SUM(mpl_amount_rubles)`) сортируется и фильтруется HAVING-диапазоном наравне с `total_amount` (`AGG_SORT_EXPR`/`AGG_FILTER_EXPR` в `fr_validation_repository.py`).

Тулкит таблицы (`static/js/shared/datatable/`, доменно-агностичен) получил ещё два opt-in режима фильтра колонки на общей попап-оболочке (`DataTable._openPopover`/`_closePopover`, тот же паттерн, что у date-попапа): `col.filterPicker: 'checkbox'` — мультивыбор по `col.filterOptions: [{value, label, short?}]` (пустой набор галочек снимает фильтр, спек `{op:'in'}`) и `col.filterPicker: 'numrange'` — диапазон «от/до» по числу (спек `{op:'range', cast:'numeric'}`, оба пустых поля снимают фильтр). `ColumnVisibility.mount({..., onApi})` отдаёт вызывающему `api.sync()` — синхронизирует чекбоксы уже открытой панели с текущим `viewState` без переоткрытия (нужно, когда видимость колонок меняется программно). ЦКФР использует оба пикера (чекбокс на `tb_breakdown` вместо прежнего словарного `filterResolve`, диапазон на `total_amount`/`total_mpl_amount`) и `onApi` — для мгновенной синхронизации панели при переключении безусловного радио «Чипы ↔ Колонки по ТБ» (`preContent`-секция панели).
