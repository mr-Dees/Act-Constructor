...
sequenceDiagram
    ...
    loop Adaptive backoff (poll_min..poll_max сек)
        ...
        P->>S: poll_once(...) — map_answer_to_blocks → finalize / upsert reasoning
...
**Ключевые модули:**
- `app/domains/chat/services/agent_channel.py` — `AgentChannelService` (`submit`, `poll_once`, `mark_timeout`, `get_queue_details`); `map_answer_to_blocks` (порядок: reasoning из `metadata.reasoning` (legacy `thinking`) → text → buttons (block_id `{id}:btn:0`) → media image/file → error), `build_timeout_error_block`.
...
| `AgentChannelService.mark_timeout` | `mark_timeout(question_uid, reason='claim'|'answer')` — дописывает error-блок (`build_timeout_error_block(reason)`; код `agent_claim_timeout` / `agent_answer_timeout`) и переводит черновик в `failed`; вопрос в шине best-effort закрывается `failed` |
