...
| `chat_agent_messages_bus.status` | Распределение `pending`/`in_progress`/`complete`/`error`/`timeout` — алёрт на залипшие |

### 3.2. Алерты (рекомендации)

| Сигнал | Запрос | Когда дёргать |
|---|---|---|
| Зависшие запросы | `SELECT count(*) FROM chat_agent_messages_bus WHERE role='user' AND status IN ('pending','in_progress') AND updated_at < now() - interval '15 minutes'` | > 0 |
| Спайки timeout'ов | `SELECT count(*) FROM chat_agent_messages_bus WHERE status='timeout' AND created_at > now() - interval '1 hour'` | > 5/час |
