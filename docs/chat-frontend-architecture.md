...
- **`chat-stream.js`** — poll-клиент (не SSE). Объект `ChatStream`
  ...
  (`chat-stream.js:26`) и `pollMessage(conversationId, messageId, options)`
  (`chat-stream.js:76`). `sendAndPoll` строит `FormData` (`message`, `domains`,
  `agent_mode`, `files[]`), делает `POST .../messages` → читает JSON
  `{message_id}`, затем зовёт `pollMessage`, который опрашивает
  `GET .../messages/{message_id}` каждые 1.5 с (`INTERVAL`) до статуса
  `complete`/`failed` или таймаута (`TIMEOUT_MS = 11 мин`, чуть больше
  серверного `answer_timeout`). Колбэки `onReady(msg)` / `onError(err)`.
  Отмена — через внешний `AbortSignal` (`signal`), который `ChatMessages`
  хранит в своём `_pollController`; `abort()` оставлен как no-op для
  совместимости. `pollMessage` переиспользуется при reload/switch посреди
  ожидания — для возобновления опроса streaming-сообщения. Есть и
  `sendJson` (`chat-stream.js:150`) для разовых не-polling вызовов.
...
- **`chat-messages.js`** — оркестратор отправки/опроса и рендер user/assistant
  ...
  чтобы typing-bubble не зависал. Запоминает welcome-сообщение как DOM-узел
...
5. **`ChatStream.sendAndPoll`** (`chat-stream.js:26-63`):
   ...
   - передаёт управление `pollMessage`, который опрашивает
     `GET .../messages/{message_id}` каждые 1.5 с до статуса
     `complete`/`failed`.
...
6. **`ChatMessages._renderReadyMessage`** (`chat-messages.js:197-211`) —
   ...
   затем рендерит блоки **целиком**: для `complete` — через
   `ChatRenderer.typeOutBlocks` (декоративный эффект печати: `text`/`reasoning`
   анимируются посимвольно, остальные типы рисуются мгновенно), для `failed` —
   через `ChatRenderer.renderBlocks` без анимации + класс
   `chat-message--failed`.
...
8. **Resume при разрыве/switch'е.** Если вкладку перезагрузили или
   ...
   (`chat-messages.js:393-443`) ставит typing-bubble и возобновляет опрос
   тем же `ChatStream.pollMessage(conversationId, msg.id, ...)`. Источник
   истины — БД: фоновый `AgentChannelPoller` независимо от фронта поллит
   bus-таблицу `agent_messages` и финализирует черновик `chat_messages`
   (`AgentChannelService.try_finalize` → `complete`/`failed`). Фронт лишь
...
2. **Опрос.** `GET /api/v1/chat/conversations/{cid}/messages/{message_id}`
   каждые 1.5 с до терминального статуса. Поля ответа: `id`, `role`, `status`,
   `content` (массив блоков). Статусы: `streaming` (ответ ещё готовится),
   `complete`, `failed`.

Ответ приходит **целиком** — токен-стриминга нет. `ChatRenderer.typeOutBlocks`
рисует его с декоративным эффектом печати (см. ниже). Для `failed`-сообщений
блоки (обычно один `error`-блок) рисуются мгновенно через `renderBlocks`.
...
кладёт вопрос в bus-таблицу `agent_messages`; фоновый `AgentChannelPoller`
поллит шину и через `AgentChannelService.try_finalize` мапит ответ агента в
блоки и финализирует черновик (`complete`/`failed`). Фронт всё это время
просто опрашивает GET до терминального статуса.

### Декоративный «эффект печати»
...
| `ChatStream._buildUrl` | `chat-stream.js:207` |
