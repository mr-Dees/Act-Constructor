...
пунктуацию выделенного текста через LLM (перенос наработки D17). Открывает
...
(`_expandRangeOutOfMarkers`). Кнопки гасят `mousedown` (B-40), выделение держится клоном
`Range`. Бэкенд — `POST /api/v1/chat/text-actions/correct` (домен chat, дословный промпт
...
**компактная колонка** (`width: 19rem`): ряд поиска (поле + счётчик + ✕), ряд
параметров (тумблеры `Aa`/`Слово`/`.*` слева, навигация `‹ ›` справа), ряд
замены с `flex-wrap` (кнопки переносятся под поле на узкой панели) — «выше, а не
шире».
