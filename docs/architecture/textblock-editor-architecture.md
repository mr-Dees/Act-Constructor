...
- **Легаси пробельные тела сносок экспортируются как есть** (EXP-3):
  сноска с телом из одних пробелов даёт Word-сноску, которую фронт не
  нумерует; трим на сохранении покрывает новые правки, старые данные не
  переписываются.
