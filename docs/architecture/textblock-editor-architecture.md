...
`static/js/constructor/text-actions/` (`corrector-popover.js`, `text-actions-client.js`);
...
`isCapsuleNode(node)`/`isZeroWidthNode(n)` (`textblock-core.js:18-42`) — pure-
функции, единый источник истины; прототипные `_isCapsule`/`_isZeroWidthNode`
(`textblock-editor.js:127-150`) — тонкие делегаты для существующих вызовов
...
чинилась даже перезагрузкой (`_isZeroWidthNode`, `textblock-editor.js:148-158`).
...
**Кнопка «Корректор» (`✨`, `data-command="improveText"`).** Правит орфографию/
пунктуацию выделенного текста через LLM (перенос наработки D17). Открывает поповер
`CorrectorPopover` (`static/js/constructor/text-actions/corrector-popover.js`) —
«выпадашку от кнопки»: вложена в группу тулбара (как меню размера шрифта), поле
ресайзабельно (`makeResizablePanel`). Показывает word-diff (`DiffEngine._wordDiff` →
`<ins>/<del>`) исправленного текста; кнопки Принять / Отклонить / Перегенерировать.
«Принять» → `ActSearchEngine.replaceRange` (§12) + `finalizeEdit` (тот же сток
персиста, что печать/find-bar); пересечение границы капсулы → предупреждение, без
правки. Кнопки поповера гасят `mousedown` (B-40), выделение держится клоном `Range`.
Бэкенд — `POST /api/v1/chat/text-actions/correct` (домен chat, дословный промпт
...
(`_buildPasteFragment`, `textblock-editor.js:701-710`): **свой буфер** (метка
