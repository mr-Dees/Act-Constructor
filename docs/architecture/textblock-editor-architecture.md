...
шапка файла) — задел под будущие цели (ячейки таблиц), не трогая движок. v1
реализует только `TextBlockSearchTarget` (обёртка над `.textblock-editor`):
`collectRuns()` делегирует в кэш движка (12.3), `persist()` = ЕДИНСТВЕННЫЙ
вызов `textBlockManager.finalizeEdit(editor)` для этой цели — сам движок
`finalizeEdit` никогда не зовёт, это ответственность вызывающего (`find-bar.js`).
...
- Текстовый узел с предком-капсулой не встречается — DFS в капсулу не
  спускается вовсе, её текст физически не попадает ни в один пробег.
  Совпадение поэтому **математически не может** зайти внутрь капсулы или
  пересечь её границу — не постфактум-фильтр, а свойство сборки.
- Чистый zero-width узел (`isZeroWidthNode` — caret-guard `U+FEFF`, якорь
  размера `U+200B`, пустой) пропускается, **не разрывая** пробег.
- Капсула, `<br>`, `<img>` — атомы/void: разрывают пробег. `<img>` тоже: без
...
- Прочий inline/блок (`b`/`i`/`u`/`span`-формат) — прозрачно обходится,
  текст склеивается в текущий пробег сквозь форматирование.
