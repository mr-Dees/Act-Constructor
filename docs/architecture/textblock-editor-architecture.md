...
связность). **Обращения к модели на открытии НЕТ** — обработка стартует по клику на
нужный режим (кнопка режима = кнопка запуска), чтобы не тратить лишний вызов, когда
нужен не дефолтный режим. Открывает
