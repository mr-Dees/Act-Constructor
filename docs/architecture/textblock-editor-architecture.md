...
фокуса/выделения редактора. Панель **перетаскиваемая** за грип `⠿`
(`shared/draggable-panel.js`, тот же helper, что у поповера корректора; позиция
персистится в `act-find-bar:pos`) — базово справа вверху.
