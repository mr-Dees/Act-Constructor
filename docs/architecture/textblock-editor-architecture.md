...
- Капсула — граница пробега: DFS `flush()`-ит текущий (внешний) пробег на
  ...
  вставленный в `runs` на месте капсулы (между «до» и «после»). Сегменты
  этого пробега указывают на РЕАЛЬНЫЕ текстовые узлы внутри капсулы (не
  синтетика) — `_locate`/`_rangeFromRun` строят по нему обычный DOM `Range`
  без изменений, поэтому такое совпадение подсвечивается и матчится штатно.
  Un-replaceable гарантия НЕ ослаблена: `replaceRange`/`_hasCapsuleAncestor`
  работает по живой DOM-цепочке предков, а не по метке пробега, и по-прежнему
  бросает на любом Range с концом внутри капсулы — попытка «Заменить»/
  «Заменить всё» на `capsuleText`-совпадении отклоняется тем же путём, что и
  раньше (счётчик «пропущено» в `_replaceAll`, см. §12.7). Вложенное
  форматирование ВНУТРИ капсулы (`<span class="text-link"><b>текст</b></span>`)
  обходится в `capsuleText`-пробег транзитивно (капсулы не вкладываются друг
  в друга, см. §3) — но не покидает его пределы.
...
`ActSearchEngine.replaceRange(range, text)` — defense-in-depth: бросает, если
ЛЮБАЯ граница диапазона лежит внутри капсулы (движок никогда не строит такой
диапазон сам, но публичный API мог бы получить произвольный Range).
...
- **Замена** — `_spliceFootnoteBodyText(footnoteEl, start, end, replacement)`
  ...
  Пустое тело сноски после замены обрабатывается уже существующей общей
  веткой `validateAndRepairCapsules` (§5): разворачивает капсулу в
  plain-текст на следующей записи в `content` — новой логики не заводилось.
