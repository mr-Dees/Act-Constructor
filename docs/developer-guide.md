...
3. Если профиль не поддерживает streaming — добавить guard в `stream_loop.py` `streaming_enabled and profile != "<new>"`. **Альтернатива** (TODO в backlog): вынести `is_streaming_supported: bool` в `ChatDomainSettings`.
...
Orchestrator._parse_client_action_result(raw, message_id, idx)
    ↓ если type == "client_action" → block_id переписан на f"{message_id}:ca:{i}" → block в emitted_blocks
