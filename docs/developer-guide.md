...
2. Форвард (`always` либо решение оркестратора в `adaptive`): `AgentChannelService.submit` INSERT'ит вопрос в `agent_messages` (`role='user'`, `status='pending'`) + создаёт черновик `chat_messages` (`status='streaming'`, `agent_ref=<uid вопроса>`) + подписывает его в `AgentChannelPoller`.
3. POST отдаёт `{message_id}`. Фронт поллит `GET /messages/{message_id}` до терминального статуса.
4. Фоновый `AgentChannelPoller` поллит шину; на ответ агента `AgentChannelService.try_finalize` маппит ответ в блоки (`map_answer_to_blocks`) и финализирует черновик (`status='complete'`). По истечении `ANSWER_TIMEOUT_SEC` — `mark_timeout` (`status='failed'`).
...
**Ключевые модули:**
- `app/domains/chat/services/agent_channel.py` — `AgentChannelService` (`submit`, `try_finalize`, `mark_timeout`); `map_answer_to_blocks` (порядок: reasoning из `metadata.thinking` → text → buttons (block_id `{id}:btn:0`) → media image/file → error), `build_timeout_error_block`.
...
1. Удалять только записи в терминальном статусе (`status IN ('complete', 'error', 'timeout')`).
...
1. `reasoning` — из `metadata.thinking` (block_id `f"{id}:reasoning:0"`);
2. `text` — из `content`;
...
| Кто | Что делает |
|---|---|
| `AgentChannelService.submit` | INSERT вопроса (`role='user'`, `status='pending'`) в `agent_messages` + создание черновика `chat_messages` (`status='streaming'`, `agent_ref=<uid вопроса>`) |
...
| `AgentChannelService.try_finalize` | Маппит ответ агента (`map_answer_to_blocks`) в блоки, финализирует черновик `chat_messages` (`status='complete'`) |
