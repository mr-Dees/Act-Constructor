...
--    сохраняет ассистент-сообщение, не дожидаясь срабатывания
--    fallback-таймера. Без `final`-события раннер всё равно сохранит
--    сообщение (через периодический poll раз в poll_min_interval_sec),
--    но с задержкой до ~event_timeout. payload может быть пустым `{}`.
-- ============================================================================
...
-- одной транзакции с save_assistant_message (optimistic locking).
